...
class Net(torch.nn.Module):
	...
	def get_loss(self, train_set, report_loss, report_examples, t_offset1, t_offset2, nt_offset, loss):
		beg = end = 0
		for loss_batch_examples in train_set.batch_iter(batch_size=self.batch_size, shuffle=False, sort=False):
			_, _, _, record_loss = self.train_iter_process(loss_batch_examples, report_loss, report_examples,
														   t_offset1,
														   t_offset2, nt_offset)
			...
			loss[beg:end].copy_(record_loss.view(-1).detach())
	...
	def observe(self, task_data, t):
		...
		while True:
			...
			if epoch == 1:
				self.get_loss(train_set, report_loss, report_examples, t_offset1, t_offset2, nt_offset, first_loss)

			if epoch == self.args.max_epoch:
				self.get_loss(train_set, report_loss, report_examples, t_offset1, t_offset2, nt_offset, last_loss)
			...
			for batch_examples in train_set.batch_iter(batch_size=self.batch_size, shuffle=True):
				...
				if len(self.observed_tasks) > 1:
					for tt in range(len(self.observed_tasks) - 1):
						...
						_, _, replay_loss, record_loss = self.train_iter_process(memory_examples, 0, 0, p_t_offset1, p_t_offset2,
																	p_nt_offset)
				...
				report_loss, report_examples, loss, record_loss = self.train_iter_process(batch_examples, report_loss,
				...
				t_action_loss.backward(retain_graph=True)
