...
class Net(torch.nn.Module):
	...
	def __init__(self,
		...
		self.aug_grads = torch.Tensor(sum(self.grad_dims), n_tasks)

		if args.use_cuda:
			self.grads = self.grads.cuda()

			self.aug_grads = self.aug_grads.cuda()
		...
		while True:
			...
			for batch_examples in train_set.batch_iter(batch_size=self.batch_size, shuffle=True):
				...
				if len(self.observed_tasks) > 1:
					...
					replay_grad = self.grads.index_select(1, indx)
					...
					if self.args.augment and train_iter % self.args.augment == 0:
						for tt in range(len(self.observed_tasks) - 1):
							...
							memory_batch_examples = self.memory_data[past_task]

							replay_batch_examples = generate_concat_samples(memory_batch_examples,
								                                                 batch_examples)

							_, _, replay_loss = self.train_iter_process(
								replay_batch_examples, 0, 0, p_t_offset1, p_t_offset2,
								p_nt_offset)

							replay_loss.backward()

							store_grad(self.parameters, self.aug_grads, self.grad_dims,
							           past_task)

						dotp = torch.bmm(self.aug_grads.index_select(1, indx).t().unsqueeze(1),
						                self.grads.index_select(1, indx).t().unsqueeze(-1)).squeeze()

						mask = (dotp < 0)
						# print (mask)
						replay_grad = self.aug_grads.index_select(1, indx) * mask + self.grads.index_select(1, indx) * ~mask

					overwrite_grad(self.parameters, torch.sum(replay_grad, dim=-1),
					               self.grad_dims)
