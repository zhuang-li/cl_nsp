...
class TemplateDB(object):
    ...
    def __init__(self):
        ...
        #self.prefix2treeSet = dict()
        #self.processed_prefix = dict()
        ...
        for tid, ref_bfs_tree in occur_list:
            ...
            if ref_bfs_tree.depth() > 0:
                ...
                if len(ref_children) > len(key_children):
                    # check if every children is an indexed tree
                    all_indexed = True
                    ...
                    if use_all_subtrees:
                        ...
                        for child in ref_children:
                            if child in key_children_copy:
                                ...
                                all_indexed = False
                            #print('{} is not indexed'.format(subtree))

                    # expand with other children
                    if all_indexed:
                        #print('{} is a candidate.'.format(ref_bfs_tree))
                        included_children = key_children.copy()
                        full_ref_tree = ref_bfs_tree.root.bfs_tree()
                        new_tree = full_ref_tree.clone()
                        #new_children = new_tree[0][0]
                        #new_children.clear()
                        #for ref_tree,ref_child in zip(bfs_tree_list, ref_children):
                        #    new_children.append(ref_tree.clone().root)
                        if not new_tree in existing_treeSet:
                            ...
                            #if '_const' in new_tree.root.head:
                            #    print(new_tree)
                            while j < occurList.size():
                             ...
                             #   if '_const' in new_tree.root.head and ref_tree_next_repr !=  ref_repr:
                             #       print('!!{} != {}'.format(ref_tree_next_repr, ref_repr))
    ...
    for bfs_tree in tree_list:
        if not template_db.is_tree_extended(bfs_tree):
            ...
            for tree_key, occurList in new_tree2occurList.items():
                if not (occurList.size() < pre_occurList.size()):
                    assert (occurList.size() == pre_occurList.size())
            ...
            for new_tree, occurList in final_tree2occurList.items():
                ...
                #template_db.index_tree_with_prefix(new_tree)
            if len(final_tree2occurList) > 0:
                del template_db.tree2occurList[bfs_tree]
                #template_db.rm_tree_from_prefix(bfs_tree)


def normalize_trees(trees, train_temp_db=None):
    temp_db = TemplateDB()
    if train_temp_db is not None:
        for tree, occlist in train_temp_db.tree2occurList.items():
            temp_db.tree2occurList[tree] = OccurList()
    ...
    while tree_updated:
        ...
        copy_list = temp_db.tree2occurList.copy()
