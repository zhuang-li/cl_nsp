...
class RuleVertex(TreeVertex):
    ...
    def copy_no_link(self):
        ...
        new_v.position = self.position
        new_v.is_auto_nt = self.is_auto_nt
        new_v.original_var = self.original_var
        new_v.original_entity = self.original_entity
        new_v.is_grammar_vertex = self.is_grammar_vertex
        new_v.finished = self.finished
        new_v.created_time = self.created_time
        return new_v
    ...
    def add_head_token(self, rule_vertex):
        self.head = self.head + " " + rule_vertex.head
...
class CompositeTreeVertex(TreeVertex):
    ...
    def is_variable(self):
        if isinstance(self.vertex, RuleVertex):
            return (not self.vertex.has_children()) # and (self.vertex.head == self.var_name or self.vertex.head.startswith(SLOT_PREFIX) or self.head.startswith(TYPE_SIGN))
        else:
            return False
...
def dfs_recursive_search(vertex, sym_list, visited, display_auto_nt=False, show_position = False):
        if not vertex.is_auto_nt or display_auto_nt:
            #if show_position:
            #    sym_list.append('{}@{}'.format(grammar.head, grammar.position))
            #else:
        ...
        #print("{} :- {} ## ".format(grammar.head, [c.head for c in grammar.children]), sym_list)
        i = 0
        last_child = None
        for child in vertex.children:
