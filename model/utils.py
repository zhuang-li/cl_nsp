...
class GloveHelper(object):
    def __init__(self, glove_file, embed_size):
        ...
        #embeds = np.zeros((10000, embed_size), dtype='float32')
    ...
    def embeddings(self):
        with open(self.glove_file, 'r') as f:
            for line in f:
                tokens = line.split()
                if not len(tokens) == 2:
                    word, embed = " ".join(tokens[:-self.embed_size]), np.array([float(tok) for tok in tokens[-self.embed_size:]])
                    #print(word)
                    yield word, embed
    ...
    def load_to(self, embed_layer, vocab):
        ...
        for word, embed in self.embeddings:
            #print(word)
            if word in vocab.token2id and vocab.token2id[word] in word_ids:
