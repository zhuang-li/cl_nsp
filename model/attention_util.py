...
import torch
from grammar.consts import SLOT_PREFIX, VAR_NAME, SPECIAL_TOKENS
...
class AttentionUtil(object):
    ...
    @staticmethod
    def get_candidate_tokens_with_tgt_tokens(src_tokens, tgt_tokens):
        ...
        for src_idx, src_token in enumerate(src_tokens):
            ...
            for tgt_token in tgt_tokens:
                tgt_token = tgt_token.lower()
                if not (tgt_token.startswith('[') or tgt_token in SPECIAL_TOKENS):
                    if len(src_token) >= 2 and (tgt_token.startswith(src_token)):
                        tokens_to_attend[src_idx] = src_token
