...
def prepare_nlmap(train_file, test_file, test_length = 0, diff_length = 0):
    vocab_freq_cutoff = 0
    train_set, train_temp_db = produce_data(train_file, data_format, lang, turn_v_back=False,parse_mode='topdown', frequent=500)
    ori_test_set, test_temp_db = produce_data(test_file, data_format, lang, turn_v_back=False,parse_mode='topdown', frequent=200)

    print (len(ori_test_set))
    print (len(ori_test_set)/3)

    test_len = int(len(ori_test_set) - len(ori_test_set)/3)

    test_set = ori_test_set[:test_len]

    dev_set = ori_test_set[test_len:]
...
if __name__ == '__main__':
    ...
    for data_format in data_formats:
        ...
        test_length, diff_length = prepare_nlmap(train_file,test_file, test_length, diff_length)
