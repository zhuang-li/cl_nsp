...
def produce_data(data_filepath, data_type, lang, turn_v_back=False, normlize_tree=True, rule_type="ProductionRuleBLB", parse_mode='bottomup', previous_src_list = None, previous_action_seq = None, frequent = 0, use_white_list = True, train_temp_db = None, paraphrase = False):
    ...
    with open(data_filepath) as json_file:
        for line in json_file:
            ...
            tgt = line_list[1]
            if paraphrase:
                text = "paraphrase: " + src + " </s>"

                encoding = paraphrase_tokenizer.encode_plus(text, padding=True, return_tensors="pt")
                ...
                for output in outputs:
                    paraphrase_text = paraphrase_tokenizer.decode(output, skip_special_tokens=True,
                                                             clean_up_tokenization_spaces=True)

                    get_src_tgt(paraphrase_text, tgt, nltk_lemmer, src_list, tgt_code_list, ori_tgt_code_list)
    ...
    json_file.close()
    """
    test_length = len(src_list)

    if train_file_path is not None:
        with open(train_file_path) as json_file:
            for line in json_file:
                line_list = line.split('\t')
                src = line_list[0].lower()
                tgt = line_list[1]
